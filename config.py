...
REQUIRE_LIVE_TRAILING_STOP = _get_bool("REQUIRE_LIVE_TRAILING_STOP", True)
...
ENABLE_LONG_REVERSAL_REBOUND = _get_bool("ENABLE_LONG_REVERSAL_REBOUND", True)
LONG_REVERSAL_LOOKBACK_CANDLES = _get_int("LONG_REVERSAL_LOOKBACK_CANDLES", 12)
LONG_REVERSAL_MIN_DROP_PCT = _get_float("LONG_REVERSAL_MIN_DROP_PCT", 2.2)
LONG_REVERSAL_MIN_BOUNCE_FROM_LOW_PCT = _get_float("LONG_REVERSAL_MIN_BOUNCE_FROM_LOW_PCT", 1.2)
LONG_REVERSAL_MAX_BOUNCE_FROM_LOW_PCT = _get_float("LONG_REVERSAL_MAX_BOUNCE_FROM_LOW_PCT", 5.8)
LONG_REVERSAL_MIN_CLOSE_POSITION = _get_float("LONG_REVERSAL_MIN_CLOSE_POSITION", 0.55)
LONG_REVERSAL_MIN_VOLUME_RATIO = _get_float("LONG_REVERSAL_MIN_VOLUME_RATIO", 1.15)
LONG_REVERSAL_MIN_RSI = _get_float("LONG_REVERSAL_MIN_RSI", 42.0)
LONG_REVERSAL_MAX_RSI = _get_float("LONG_REVERSAL_MAX_RSI", 68.0)
LONG_REVERSAL_HIGH_RSI_THRESHOLD = _get_float("LONG_REVERSAL_HIGH_RSI_THRESHOLD", 58.0)
LONG_REVERSAL_HIGH_RSI_MIN_ADX = _get_float("LONG_REVERSAL_HIGH_RSI_MIN_ADX", 35.0)
LONG_REVERSAL_REQUIRE_MACD_IMPROVING = _get_bool("LONG_REVERSAL_REQUIRE_MACD_IMPROVING", True)
LONG_REVERSAL_MIN_SCORE = _get_int("LONG_REVERSAL_MIN_SCORE", 5)
LONG_REVERSAL_STOP_LOSS_PCT = _get_float("LONG_REVERSAL_STOP_LOSS_PCT", 1.3)
LONG_REVERSAL_PARTIAL_TARGET_PCT = _get_float("LONG_REVERSAL_PARTIAL_TARGET_PCT", 1.0)
LONG_REVERSAL_TRAILING_TRIGGER_PCT = _get_float("LONG_REVERSAL_TRAILING_TRIGGER_PCT", 1.2)
LONG_REVERSAL_TRAILING_STOP_PCT = _get_float("LONG_REVERSAL_TRAILING_STOP_PCT", 0.55)
LONG_REVERSAL_USE_FIXED_STOP = _get_bool("LONG_REVERSAL_USE_FIXED_STOP", False)
...
ENABLE_SHORT_REVERSAL_REJECTION = _get_bool("ENABLE_SHORT_REVERSAL_REJECTION", True)
SHORT_REVERSAL_LOOKBACK_CANDLES = _get_int("SHORT_REVERSAL_LOOKBACK_CANDLES", 12)
SHORT_REVERSAL_MIN_RISE_PCT = _get_float("SHORT_REVERSAL_MIN_RISE_PCT", 2.2)
SHORT_REVERSAL_MIN_PULLBACK_FROM_HIGH_PCT = _get_float("SHORT_REVERSAL_MIN_PULLBACK_FROM_HIGH_PCT", 1.1)
SHORT_REVERSAL_MAX_PULLBACK_FROM_HIGH_PCT = _get_float("SHORT_REVERSAL_MAX_PULLBACK_FROM_HIGH_PCT", 5.8)
SHORT_REVERSAL_MAX_CLOSE_POSITION = _get_float("SHORT_REVERSAL_MAX_CLOSE_POSITION", 0.45)
SHORT_REVERSAL_MIN_VOLUME_RATIO = _get_float("SHORT_REVERSAL_MIN_VOLUME_RATIO", 1.15)
SHORT_REVERSAL_MIN_RSI = _get_float("SHORT_REVERSAL_MIN_RSI", 45.0)
SHORT_REVERSAL_MAX_RSI = _get_float("SHORT_REVERSAL_MAX_RSI", 62.0)
SHORT_REVERSAL_LOW_RSI_THRESHOLD = _get_float("SHORT_REVERSAL_LOW_RSI_THRESHOLD", 42.0)
SHORT_REVERSAL_LOW_RSI_MIN_ADX = _get_float("SHORT_REVERSAL_LOW_RSI_MIN_ADX", 35.0)
SHORT_REVERSAL_BLOCKED_ENTRY_HOURS_UTC = _get_int_list("SHORT_REVERSAL_BLOCKED_ENTRY_HOURS_UTC", [14])
SHORT_REVERSAL_REQUIRE_MACD_WORSENING = _get_bool("SHORT_REVERSAL_REQUIRE_MACD_WORSENING", True)
SHORT_REVERSAL_MIN_SCORE = _get_int("SHORT_REVERSAL_MIN_SCORE", 5)
SHORT_REVERSAL_STOP_LOSS_PCT = _get_float("SHORT_REVERSAL_STOP_LOSS_PCT", 1.25)
SHORT_REVERSAL_PARTIAL_TARGET_PCT = _get_float("SHORT_REVERSAL_PARTIAL_TARGET_PCT", 1.0)
SHORT_REVERSAL_TRAILING_TRIGGER_PCT = _get_float("SHORT_REVERSAL_TRAILING_TRIGGER_PCT", 1.2)
SHORT_REVERSAL_TRAILING_STOP_PCT = _get_float("SHORT_REVERSAL_TRAILING_STOP_PCT", 0.55)
SHORT_REVERSAL_USE_FIXED_STOP = _get_bool("SHORT_REVERSAL_USE_FIXED_STOP", False)
...
# Filtro operacional por horario. Mantem long/short ativos, mas evita janelas
# UTC em que o lado vendido apresentou edge negativo no BTC 15m.
USE_ENTRY_HOUR_BLOCKS = _get_bool("USE_ENTRY_HOUR_BLOCKS", True)
BLOCKED_LONG_ENTRY_HOURS_UTC = _get_int_list("BLOCKED_LONG_ENTRY_HOURS_UTC", [])
BLOCKED_SHORT_ENTRY_HOURS_UTC = _get_int_list(
...
MARKET_STRUCTURE_GUARD_ENABLED = _get_bool("MARKET_STRUCTURE_GUARD_ENABLED", True)
MARKET_STRUCTURE_LOOKBACK = _get_int("MARKET_STRUCTURE_LOOKBACK", 32)
NEAR_RESISTANCE_BLOCK_PCT = _get_float("NEAR_RESISTANCE_BLOCK_PCT", 0.25)
NEAR_SUPPORT_BLOCK_PCT = _get_float("NEAR_SUPPORT_BLOCK_PCT", 0.25)
MIN_SPACE_TO_TARGET_PCT = _get_float("MIN_SPACE_TO_TARGET_PCT", 0.60)
MIN_RISK_REWARD_BEFORE_ENTRY = _get_float("MIN_RISK_REWARD_BEFORE_ENTRY", 1.4)

ENABLE_LIQUIDITY_SWEEP_REVERSAL_LONG = _get_bool("ENABLE_LIQUIDITY_SWEEP_REVERSAL_LONG", True)
ENABLE_LIQUIDITY_SWEEP_REVERSAL_SHORT = _get_bool("ENABLE_LIQUIDITY_SWEEP_REVERSAL_SHORT", True)
LIQUIDITY_SWEEP_LOOKBACK = _get_int("LIQUIDITY_SWEEP_LOOKBACK", 24)
LIQUIDITY_SWEEP_MIN_BREAK_PCT = _get_float("LIQUIDITY_SWEEP_MIN_BREAK_PCT", 0.15)
LIQUIDITY_SWEEP_MAX_BREAK_PCT = _get_float("LIQUIDITY_SWEEP_MAX_BREAK_PCT", 2.5)
LIQUIDITY_SWEEP_MIN_RECLAIM_PCT = _get_float("LIQUIDITY_SWEEP_MIN_RECLAIM_PCT", 0.20)
LIQUIDITY_SWEEP_MIN_LOWER_WICK_RATIO = _get_float("LIQUIDITY_SWEEP_MIN_LOWER_WICK_RATIO", 0.35)
LIQUIDITY_SWEEP_MIN_UPPER_WICK_RATIO = _get_float("LIQUIDITY_SWEEP_MIN_UPPER_WICK_RATIO", 0.35)
LIQUIDITY_SWEEP_MIN_VOLUME_RATIO = _get_float("LIQUIDITY_SWEEP_MIN_VOLUME_RATIO", 1.20)
LIQUIDITY_SWEEP_RSI_MAX_AT_LOW_SWEEP = _get_float("LIQUIDITY_SWEEP_RSI_MAX_AT_LOW_SWEEP", 45.0)
LIQUIDITY_SWEEP_RSI_MIN_AT_HIGH_SWEEP = _get_float("LIQUIDITY_SWEEP_RSI_MIN_AT_HIGH_SWEEP", 55.0)
LIQUIDITY_SWEEP_REQUIRE_MACD_IMPROVING = _get_bool("LIQUIDITY_SWEEP_REQUIRE_MACD_IMPROVING", True)
LIQUIDITY_SWEEP_REQUIRE_CONFIRMATION = _get_bool("LIQUIDITY_SWEEP_REQUIRE_CONFIRMATION", True)
LIQUIDITY_SWEEP_MAX_BOUNCE_FROM_LOW_PCT = _get_float("LIQUIDITY_SWEEP_MAX_BOUNCE_FROM_LOW_PCT", 4.0)
LIQUIDITY_SWEEP_MAX_PULLBACK_FROM_HIGH_PCT = _get_float(
    "LIQUIDITY_SWEEP_MAX_PULLBACK_FROM_HIGH_PCT",
    4.0,
)
LIQUIDITY_SWEEP_MIN_SCORE = _get_int("LIQUIDITY_SWEEP_MIN_SCORE", 5)
...
def build_runtime_strategy_snapshot(context_timeframe: Optional[str] = None) -> Dict[str, object]:
    ...
    return {
        ...
        "enable_long_reversal_rebound": bool(ENABLE_LONG_REVERSAL_REBOUND),
        "long_reversal_lookback_candles": int(LONG_REVERSAL_LOOKBACK_CANDLES),
        "long_reversal_min_drop_pct": float(LONG_REVERSAL_MIN_DROP_PCT),
        "long_reversal_min_bounce_from_low_pct": float(LONG_REVERSAL_MIN_BOUNCE_FROM_LOW_PCT),
        "long_reversal_max_bounce_from_low_pct": float(LONG_REVERSAL_MAX_BOUNCE_FROM_LOW_PCT),
        "long_reversal_min_volume_ratio": float(LONG_REVERSAL_MIN_VOLUME_RATIO),
        "long_reversal_min_rsi": float(LONG_REVERSAL_MIN_RSI),
        "long_reversal_max_rsi": float(LONG_REVERSAL_MAX_RSI),
        "long_reversal_high_rsi_threshold": float(LONG_REVERSAL_HIGH_RSI_THRESHOLD),
        "long_reversal_high_rsi_min_adx": float(LONG_REVERSAL_HIGH_RSI_MIN_ADX),
        "long_reversal_stop_loss_pct": float(LONG_REVERSAL_STOP_LOSS_PCT),
        "long_reversal_partial_target_pct": float(LONG_REVERSAL_PARTIAL_TARGET_PCT),
        "long_reversal_trailing_trigger_pct": float(LONG_REVERSAL_TRAILING_TRIGGER_PCT),
        "long_reversal_trailing_stop_pct": float(LONG_REVERSAL_TRAILING_STOP_PCT),
        ...
        "enable_short_reversal_rejection": bool(ENABLE_SHORT_REVERSAL_REJECTION),
        "short_reversal_lookback_candles": int(SHORT_REVERSAL_LOOKBACK_CANDLES),
        "short_reversal_min_rise_pct": float(SHORT_REVERSAL_MIN_RISE_PCT),
        "short_reversal_min_pullback_from_high_pct": float(SHORT_REVERSAL_MIN_PULLBACK_FROM_HIGH_PCT),
        "short_reversal_max_pullback_from_high_pct": float(SHORT_REVERSAL_MAX_PULLBACK_FROM_HIGH_PCT),
        "short_reversal_min_volume_ratio": float(SHORT_REVERSAL_MIN_VOLUME_RATIO),
        "short_reversal_min_rsi": float(SHORT_REVERSAL_MIN_RSI),
        "short_reversal_max_rsi": float(SHORT_REVERSAL_MAX_RSI),
        "short_reversal_low_rsi_threshold": float(SHORT_REVERSAL_LOW_RSI_THRESHOLD),
        "short_reversal_low_rsi_min_adx": float(SHORT_REVERSAL_LOW_RSI_MIN_ADX),
        "short_reversal_blocked_entry_hours_utc": list(SHORT_REVERSAL_BLOCKED_ENTRY_HOURS_UTC),
        "short_reversal_stop_loss_pct": float(SHORT_REVERSAL_STOP_LOSS_PCT),
        "short_reversal_partial_target_pct": float(SHORT_REVERSAL_PARTIAL_TARGET_PCT),
        "short_reversal_trailing_trigger_pct": float(SHORT_REVERSAL_TRAILING_TRIGGER_PCT),
        "short_reversal_trailing_stop_pct": float(SHORT_REVERSAL_TRAILING_STOP_PCT),
        ...
        "market_structure_guard_enabled": bool(MARKET_STRUCTURE_GUARD_ENABLED),
        "market_structure_lookback": int(MARKET_STRUCTURE_LOOKBACK),
        "near_resistance_block_pct": float(NEAR_RESISTANCE_BLOCK_PCT),
        "near_support_block_pct": float(NEAR_SUPPORT_BLOCK_PCT),
        "min_space_to_target_pct": float(MIN_SPACE_TO_TARGET_PCT),
        "min_risk_reward_before_entry": float(MIN_RISK_REWARD_BEFORE_ENTRY),
        "enable_liquidity_sweep_reversal_long": bool(ENABLE_LIQUIDITY_SWEEP_REVERSAL_LONG),
        "enable_liquidity_sweep_reversal_short": bool(ENABLE_LIQUIDITY_SWEEP_REVERSAL_SHORT),
        "liquidity_sweep_lookback": int(LIQUIDITY_SWEEP_LOOKBACK),
        "liquidity_sweep_min_break_pct": float(LIQUIDITY_SWEEP_MIN_BREAK_PCT),
        "liquidity_sweep_max_break_pct": float(LIQUIDITY_SWEEP_MAX_BREAK_PCT),
        "liquidity_sweep_min_reclaim_pct": float(LIQUIDITY_SWEEP_MIN_RECLAIM_PCT),
        "liquidity_sweep_min_lower_wick_ratio": float(LIQUIDITY_SWEEP_MIN_LOWER_WICK_RATIO),
        "liquidity_sweep_min_upper_wick_ratio": float(LIQUIDITY_SWEEP_MIN_UPPER_WICK_RATIO),
        "liquidity_sweep_min_volume_ratio": float(LIQUIDITY_SWEEP_MIN_VOLUME_RATIO),
        "liquidity_sweep_rsi_max_at_low_sweep": float(LIQUIDITY_SWEEP_RSI_MAX_AT_LOW_SWEEP),
        "liquidity_sweep_rsi_min_at_high_sweep": float(LIQUIDITY_SWEEP_RSI_MIN_AT_HIGH_SWEEP),
        "liquidity_sweep_require_macd_improving": bool(LIQUIDITY_SWEEP_REQUIRE_MACD_IMPROVING),
        "liquidity_sweep_require_confirmation": bool(LIQUIDITY_SWEEP_REQUIRE_CONFIRMATION),
        "liquidity_sweep_max_bounce_from_low_pct": float(LIQUIDITY_SWEEP_MAX_BOUNCE_FROM_LOW_PCT),
        "liquidity_sweep_max_pullback_from_high_pct": float(
            LIQUIDITY_SWEEP_MAX_PULLBACK_FROM_HIGH_PCT
        ),
        "liquidity_sweep_min_score": int(LIQUIDITY_SWEEP_MIN_SCORE),
...
SYMBOL_STRATEGY_OVERRIDE_KEYS = [
    ...
    "TREND_RESUME_SHORT_REQUIRE_BREAKDOWN_CONFIRMATION",
    "TREND_RESUME_SHORT_BLOCKED_ENTRY_HOURS_UTC",
    "MARKET_STRUCTURE_GUARD_ENABLED",
    "MARKET_STRUCTURE_LOOKBACK",
    "NEAR_RESISTANCE_BLOCK_PCT",
    "NEAR_SUPPORT_BLOCK_PCT",
    "MIN_SPACE_TO_TARGET_PCT",
    "MIN_RISK_REWARD_BEFORE_ENTRY",
    "ENABLE_LIQUIDITY_SWEEP_REVERSAL_LONG",
    "ENABLE_LIQUIDITY_SWEEP_REVERSAL_SHORT",
    "LIQUIDITY_SWEEP_LOOKBACK",
    "LIQUIDITY_SWEEP_MIN_BREAK_PCT",
    "LIQUIDITY_SWEEP_MAX_BREAK_PCT",
    "LIQUIDITY_SWEEP_MIN_RECLAIM_PCT",
    "LIQUIDITY_SWEEP_MIN_LOWER_WICK_RATIO",
    "LIQUIDITY_SWEEP_MIN_UPPER_WICK_RATIO",
    "LIQUIDITY_SWEEP_MIN_VOLUME_RATIO",
    "LIQUIDITY_SWEEP_RSI_MAX_AT_LOW_SWEEP",
    "LIQUIDITY_SWEEP_RSI_MIN_AT_HIGH_SWEEP",
    "LIQUIDITY_SWEEP_REQUIRE_MACD_IMPROVING",
    "LIQUIDITY_SWEEP_REQUIRE_CONFIRMATION",
    "LIQUIDITY_SWEEP_MAX_BOUNCE_FROM_LOW_PCT",
    "LIQUIDITY_SWEEP_MAX_PULLBACK_FROM_HIGH_PCT",
    "LIQUIDITY_SWEEP_MIN_SCORE",
]
...
class ProductionConfig:
    ...
    DEFAULT_LIVE_STOP_LOSS_PCT = _get_float("DEFAULT_LIVE_STOP_LOSS_PCT", LONG_STOP_LOSS_PCT)
    DEFAULT_LIVE_TAKE_PROFIT_PCT = _get_float("DEFAULT_LIVE_TAKE_PROFIT_PCT", LONG_TAKE_PROFIT_PCT)
    ...
    REQUIRE_LIVE_TRAILING_STOP = _get_bool("REQUIRE_LIVE_TRAILING_STOP", REQUIRE_LIVE_TRAILING_STOP)
    ...
    MIN_LIVE_ACCOUNT_BALANCE_USDT = _get_float(
    ...
    MARKET_STRUCTURE_GUARD_ENABLED = _get_bool(
        "MARKET_STRUCTURE_GUARD_ENABLED",
        MARKET_STRUCTURE_GUARD_ENABLED,
    )
    MARKET_STRUCTURE_LOOKBACK = _get_int("MARKET_STRUCTURE_LOOKBACK", MARKET_STRUCTURE_LOOKBACK)
    NEAR_RESISTANCE_BLOCK_PCT = _get_float("NEAR_RESISTANCE_BLOCK_PCT", NEAR_RESISTANCE_BLOCK_PCT)
    NEAR_SUPPORT_BLOCK_PCT = _get_float("NEAR_SUPPORT_BLOCK_PCT", NEAR_SUPPORT_BLOCK_PCT)
    MIN_SPACE_TO_TARGET_PCT = _get_float("MIN_SPACE_TO_TARGET_PCT", MIN_SPACE_TO_TARGET_PCT)
    MIN_RISK_REWARD_BEFORE_ENTRY = _get_float(
        "MIN_RISK_REWARD_BEFORE_ENTRY",
        MIN_RISK_REWARD_BEFORE_ENTRY,
    )
    ENABLE_LIQUIDITY_SWEEP_REVERSAL_LONG = _get_bool(
        "ENABLE_LIQUIDITY_SWEEP_REVERSAL_LONG",
        ENABLE_LIQUIDITY_SWEEP_REVERSAL_LONG,
    )
    ENABLE_LIQUIDITY_SWEEP_REVERSAL_SHORT = _get_bool(
        "ENABLE_LIQUIDITY_SWEEP_REVERSAL_SHORT",
        ENABLE_LIQUIDITY_SWEEP_REVERSAL_SHORT,
    )
    LIQUIDITY_SWEEP_LOOKBACK = _get_int("LIQUIDITY_SWEEP_LOOKBACK", LIQUIDITY_SWEEP_LOOKBACK)
    LIQUIDITY_SWEEP_MIN_BREAK_PCT = _get_float(
        "LIQUIDITY_SWEEP_MIN_BREAK_PCT",
        LIQUIDITY_SWEEP_MIN_BREAK_PCT,
    )
    LIQUIDITY_SWEEP_MAX_BREAK_PCT = _get_float(
        "LIQUIDITY_SWEEP_MAX_BREAK_PCT",
        LIQUIDITY_SWEEP_MAX_BREAK_PCT,
    )
    LIQUIDITY_SWEEP_MIN_RECLAIM_PCT = _get_float(
        "LIQUIDITY_SWEEP_MIN_RECLAIM_PCT",
        LIQUIDITY_SWEEP_MIN_RECLAIM_PCT,
    )
    LIQUIDITY_SWEEP_MIN_LOWER_WICK_RATIO = _get_float(
        "LIQUIDITY_SWEEP_MIN_LOWER_WICK_RATIO",
        LIQUIDITY_SWEEP_MIN_LOWER_WICK_RATIO,
    )
    LIQUIDITY_SWEEP_MIN_UPPER_WICK_RATIO = _get_float(
        "LIQUIDITY_SWEEP_MIN_UPPER_WICK_RATIO",
        LIQUIDITY_SWEEP_MIN_UPPER_WICK_RATIO,
    )
    LIQUIDITY_SWEEP_MIN_VOLUME_RATIO = _get_float(
        "LIQUIDITY_SWEEP_MIN_VOLUME_RATIO",
        LIQUIDITY_SWEEP_MIN_VOLUME_RATIO,
    )
    LIQUIDITY_SWEEP_RSI_MAX_AT_LOW_SWEEP = _get_float(
        "LIQUIDITY_SWEEP_RSI_MAX_AT_LOW_SWEEP",
        LIQUIDITY_SWEEP_RSI_MAX_AT_LOW_SWEEP,
    )
    LIQUIDITY_SWEEP_RSI_MIN_AT_HIGH_SWEEP = _get_float(
        "LIQUIDITY_SWEEP_RSI_MIN_AT_HIGH_SWEEP",
        LIQUIDITY_SWEEP_RSI_MIN_AT_HIGH_SWEEP,
    )
    LIQUIDITY_SWEEP_REQUIRE_MACD_IMPROVING = _get_bool(
        "LIQUIDITY_SWEEP_REQUIRE_MACD_IMPROVING",
        LIQUIDITY_SWEEP_REQUIRE_MACD_IMPROVING,
    )
    LIQUIDITY_SWEEP_REQUIRE_CONFIRMATION = _get_bool(
        "LIQUIDITY_SWEEP_REQUIRE_CONFIRMATION",
        LIQUIDITY_SWEEP_REQUIRE_CONFIRMATION,
    )
    LIQUIDITY_SWEEP_MAX_BOUNCE_FROM_LOW_PCT = _get_float(
        "LIQUIDITY_SWEEP_MAX_BOUNCE_FROM_LOW_PCT",
        LIQUIDITY_SWEEP_MAX_BOUNCE_FROM_LOW_PCT,
    )
    LIQUIDITY_SWEEP_MAX_PULLBACK_FROM_HIGH_PCT = _get_float(
    ...
    LIQUIDITY_SWEEP_MIN_SCORE = _get_int("LIQUIDITY_SWEEP_MIN_SCORE", LIQUIDITY_SWEEP_MIN_SCORE)
...
__all__ = [
    ...
    "ENABLE_LONG_REVERSAL_REBOUND",
    "LONG_REVERSAL_LOOKBACK_CANDLES",
    "LONG_REVERSAL_MIN_DROP_PCT",
    "LONG_REVERSAL_MIN_BOUNCE_FROM_LOW_PCT",
    "LONG_REVERSAL_MAX_BOUNCE_FROM_LOW_PCT",
    "LONG_REVERSAL_MIN_CLOSE_POSITION",
    "LONG_REVERSAL_MIN_VOLUME_RATIO",
    "LONG_REVERSAL_MIN_RSI",
    "LONG_REVERSAL_MAX_RSI",
    "LONG_REVERSAL_HIGH_RSI_THRESHOLD",
    "LONG_REVERSAL_HIGH_RSI_MIN_ADX",
    "LONG_REVERSAL_REQUIRE_MACD_IMPROVING",
    "LONG_REVERSAL_MIN_SCORE",
    "LONG_REVERSAL_STOP_LOSS_PCT",
    "LONG_REVERSAL_PARTIAL_TARGET_PCT",
    "LONG_REVERSAL_TRAILING_TRIGGER_PCT",
    "LONG_REVERSAL_TRAILING_STOP_PCT",
    "LONG_REVERSAL_USE_FIXED_STOP",
    ...
    "ENABLE_SHORT_REVERSAL_REJECTION",
    "SHORT_REVERSAL_LOOKBACK_CANDLES",
    "SHORT_REVERSAL_MIN_RISE_PCT",
    "SHORT_REVERSAL_MIN_PULLBACK_FROM_HIGH_PCT",
    "SHORT_REVERSAL_MAX_PULLBACK_FROM_HIGH_PCT",
    "SHORT_REVERSAL_MAX_CLOSE_POSITION",
    "SHORT_REVERSAL_MIN_VOLUME_RATIO",
    "SHORT_REVERSAL_MIN_RSI",
    "SHORT_REVERSAL_MAX_RSI",
    "SHORT_REVERSAL_LOW_RSI_THRESHOLD",
    "SHORT_REVERSAL_LOW_RSI_MIN_ADX",
    "SHORT_REVERSAL_BLOCKED_ENTRY_HOURS_UTC",
    "SHORT_REVERSAL_REQUIRE_MACD_WORSENING",
    "SHORT_REVERSAL_MIN_SCORE",
    "SHORT_REVERSAL_STOP_LOSS_PCT",
    "SHORT_REVERSAL_PARTIAL_TARGET_PCT",
    "SHORT_REVERSAL_TRAILING_TRIGGER_PCT",
    "SHORT_REVERSAL_TRAILING_STOP_PCT",
    "SHORT_REVERSAL_USE_FIXED_STOP",
